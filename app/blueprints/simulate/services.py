...
from app.models import Email, Template, UserAction, CampaignTarget, SmsAction
...
def ensure_default_templates():
    """Create default templates from existing seeded emails when none exist."""
    if Template.query.count() > 0:
        return 0

    emails = Email.query.order_by(Email.id.asc()).all()
    if not emails:
        return 0
    ...
    for email in emails:
        template = Template(
            name=f"Template: {email.subject[:60]}",
            sender=email.sender,
            subject=email.subject,
            body=email.body,
            is_phishing=bool(email.is_phishing),
            difficulty=email.difficulty,
            link_text=email.link_text or 'Open link',
            link_url=email.link_url or '#',
...
# ============================================================
# These functions ensure all required columns exist in the database.
# They are called on app startup and silently succeed if columns already exist.
# Purpose: Handle graceful schema evolution during development.
# Note: All columns are defined in SQLAlchemy models; these serve as a safety net.
# For production migrations, use: alembic upgrade head
# ============================================================
