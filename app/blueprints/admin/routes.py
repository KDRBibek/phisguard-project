...
def require_admin(f):
    def wrapper(*args, **kwargs):
        ...
        if not token:
            logger.warning(f'Admin access attempt without token from {request.remote_addr}')
            return jsonify({'error': 'no token provided'}), 401
        
        is_valid = is_admin_token(token)
        if not is_valid:
            logger.warning(f'Admin access attempt with invalid token from {request.remote_addr}')
...
def api_campaigns():
    ...
    print(f"targets count: {len(target_ids)}")
    print(f"department_filter: {department_filter}")
    print(f"role_filter: {role_filter}")
    print(f"state: {requested_state}")
    print(f"================================\n")
    ...
    if not name or not template_id:
        error_msg = f"missing name={name}, template_id={template_id}"
        print(f"✗ Campaign creation failed: {error_msg}")
    ...
    print(f"Resolved targets count: {len(targets)}")
    print(f"Target IDs from form: {target_ids}")
    print(f"Department filter: {department_filter}")
    print(f"Role filter: {role_filter}")
    
    if not targets:
        error_msg = f"no targets matched selection (ids={target_ids}, dept={department_filter}, role={role_filter})"
        print(f"✗ Campaign creation failed: {error_msg}")
        return jsonify({'error': error_msg}), 400
