...
# Load environment variables BEFORE any app imports that depend on them
load_dotenv()

from flask import Flask
from app.config import Config
from app.extensions import db, login_manager
from app.models import Email, User, Session
...
def create_app(config_class=Config):
    ...
    with app.app_context():
        ...
        # 1. db.create_all() creates tables from SQLAlchemy model definitions (source of truth)
        # 2. Alembic migrations (migrations/ folder) available for reference but not auto-applied
        # 3. ensure_*_column() helpers patch potential schema gaps (handles dev db migrations gracefully)
        # 4. seed_emails() pre-loads sample phishing scenarios on first startup
        db.create_all()
        try:
